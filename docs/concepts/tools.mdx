...
klaw includes 12 powerful tools out of the box:
...
<CardGroup cols={2}>
  ...
  <Card title="delegate" icon="users">
    ...
    independently with its own tool set and returns a text result.

    ```json
    {
      "tool": "delegate",
      "input": {
        "task": "Read all Go files in /src and summarize the architecture",
